...
thumbnails:
  ...
  maxCacheMiB: 0             # max disk usage for the thumbnail cache; oldest files evicted first; 0 = unlimited
...
transcode:
  ...
  maxCacheMiB: 0             # max disk usage for the transcode cache; oldest files evicted first; 0 = unlimited
...
ai:
  ...
  analyzeTimeout: 0            # per-item deadline for the sidecar analyze call; 0 = default (5m); increase for large video files (e.g. 10m)
